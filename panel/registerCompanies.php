...
<main id="main" class="main">
    ...
    <div class="modal" id="myModal">
        ...
        <div class="modal-dialog modal-lg">
            <!-- Cambia la clase modal-dialog a modal-xl para hacer el modal más grande -->
                ...
                <section class="section">
                    <div class="card">
                        <div class="card-body"> <br>
                            ...
                            <form class="row g-3" id="form-empresa">
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        ...
                                        <label for="floatingName">Nombre de la empresa</label>
                                    </div>
                                </div>

                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="number" class="form-control" id="telEmpresa" name="telEmpresa"
                                            placeholder="Nombre de la sede"
                                            onKeyUp="this.value=this.value.toUpperCase()" required>
                                        <label for="floatingName">Telefono de la empresa</label>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-8">
                                    <div class="form-floating">
                                        <input type="text" class="form-control" id="dirEmpresa" name="dirEmpresa"
                                            placeholder="Nombre de la sede"
                                            onKeyUp="this.value=this.value.toUpperCase()" required>
                                        <label for="floatingName">Direccion de la empresa</label>
                                    </div>
                                </div>
    ...
    <div class="row">
        <div class="col-4">
            ...
            <i class="bi bi-plus-circle" aria-hidden="true"></i> Crear Nueva Empresa
            </button>
        </div>
    </div>
    <br>
...
tabladata = $("#datatable-responsive").DataTable({
    ...
    ordering: false,
    "ajax": {
        url: './capaDatos/listCompanies.php',
        type: "GET",
        dataType: "json"
    },
    "columns": [{
        ...
        {
            "defaultContent": '<button type="button" class="btn btn-primary btn-sm btn-editar" data-id="{{IdEmpresa}}"><i class="bx bxs-edit-alt"></i></button>',
            "orderable": false,
            "searchable": false,
            "width": "90px"
        }
...
$("#form-empresa").validate({
    ...
    messages: {
        nomEmpresa: "El nombre de la empresa es obligatorio.",
        telEmpresa: "Ingrese un numero valido.",
        dirEmpresa: "La dirección de la empresa es obligatorio.",
...
function abrirModal(dataJson, isEditing) {
    ...
    if (dataJson != null) {
        ...
        $("#id").val(dataJson.IdEmpresa);
        $("#nomEmpresa").val(dataJson.Nombre);
        $("#telEmpresa").val(dataJson.Telefono);
        $("#dirEmpresa").val(dataJson.Direccion);
...
function Guardar() {
    ...
    if ($("#form-empresa").valid()) {
        ...
        $.ajax({
                type: 'POST',
                url: './CapaDatos/CreateCompanie.php',
                data: CreateCompanie,
                dataType: 'json',
                async: true,
            })
            .done(function(res) {
                console.log(res);
                if (CreateCompanie.id == 0) {
                    if (res && res.resultado !== undefined && res.resultado != 0) {
                        CreateCompanie.id = res.resultado;
                        Swal.fire("¡Buen Trabajo!", res.mensaje, "success");
                        $("#myModal").modal("hide");
                        tabladata.ajax.reload();
                    } else {
                        Swal.fire("Revise los datos ingresados", res.mensaje, "info");
...
function resetForm() {
    ...
    $("#form-empresa .form-control").removeClass("is-invalid is-valid");
    $(".form-activa").hide();
    $(".modal-title").text("Crear Empresa");
}
